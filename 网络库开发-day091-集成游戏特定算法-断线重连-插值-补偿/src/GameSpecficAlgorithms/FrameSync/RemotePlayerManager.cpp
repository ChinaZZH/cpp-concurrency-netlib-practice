...
void RemotePlayerManager::UpdateRemoteState(uint32_t player_id, Fixed x, Fixed y, uint64_t timestamp_ms)
{
...
bool RemotePlayerManager::GetRenderPosition(uint32_t player_id, uint64_t now_ms, Fixed& out_x, Fixed& out_y) const
{
    ...
    RemoteStateSnapshot snapShot = smoother.GetRenderState(now_ms);
