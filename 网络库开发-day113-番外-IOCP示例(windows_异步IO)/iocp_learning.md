...
|-------|-----------------|---------------------|
|I/O 模型	| Reactor（同步非阻塞 应用层主动读写）|	Proactor（异步 内核完成读写后通知）|
|通知内容	|“可以读/写了”（就绪通知）	|“已经读/写完了”（完成通知）|
|数据拷贝	| 应用层收到通知后调用 recv/send 主动拷贝 |	内核完成拷贝，用户态直接使用缓冲区数据|
|工作线程职责	| 监听事件 → 读写 → 处理业务 |	发起 I/O 请求 → 等待完成通知 → 直接使用数据|
|适用平台|	Linux	| Windows |
|并发模型|单 Reactor 多线程 / 主从 Reactor|单完成端口 + 多工作线程|
|典型代表|Nginx、Redis、libevent、muduo|许多 Windows 高性能服务程序|

## 一句话总结：

-- 1. epoll（Reactor）：通知你“可以读/写了”，但数据还没动，你得自己动手。

-- 2. IOCP（Proactor）：通知你“已经读/写完了”，数据已经在缓冲区里了，你直接用。


## 本质差异：数据流向
```text
epoll（Reactor）：
  应用程序 → epoll_wait 等待事件 → 收到“可读”通知 → 调用 recv 从内核拷贝数据 → 处理数据
  应用程序 → 处理完数据 → 调用 send 拷贝数据到内核 → 等待下一次事件

IOCP（Proactor）：
  应用程序 → 投递 WSARecv（异步读） → 内核自动将数据拷贝到用户缓冲区 → 完成端口通知 → 直接使用数据
  应用程序 → 投递 WSASend（异步写） → 内核自动从用户缓冲区拷贝数据 → 完成端口通知 → 发送完成
```

-- 1. 关键差异：在 Reactor 模型中，数据拷贝是应用程序主动发起的（recv/send 系统调用）；

-- 2. 在 Proactor 模型中，数据拷贝是内核在 I/O 完成后自动完成的，应用程序直接使用已经就绪的数据。

## 差异点
|IOCP 中做的事	| epoll 中的对应做法|
|----------------------|---------------------|
|投递 WSARecv 后立即返回，等待完成通知|	epoll_wait 返回可读事件后，主动调用 recv|
|工作线程 GetQueuedCompletionStatus 等待完成事件|	工作线程 epoll_wait 等待就绪事件|
|数据已在 recv_io.buffer 中，直接使用|	收到可读通知后，需要调用 recv 填充用户缓冲区|
|AcceptEx 异步接受连接，完成后通知|	accept 同步调用，或 accept4 非阻塞配合 epoll|