...
## 1.内存模型必要性
 ...
 (2).除了指令重排内存序也在多个线程之间同步共享数据指定了强弱关系。
...
## 4.自旋锁
 (1).使用 std::atomic_flag或std::atomic<bool>来实现自旋锁，展示了 acquire/release 的典型用法。 

(2).自旋锁通过原子交换指令循环等待，适合临界区极短的场景(临界区的运行时间小于线程间切换的时间)。
...
2. 无锁栈引用计数解法(隐式版本号): 通过引用计数方法中shared_ptr中的控制块的地址不同。
   使用store()、exchange()、compare_exchange_weak/strong()、fetch_add() 的时候就是把 (内存地址+控制块地址) 一起打包跟栈顶的 (内存地址+控制块地址) 做比较来进行ABA防御。
...
4. 从我们的基准测试结果来看，无锁线程池都要优于有锁竞争的线程池.工作线程竞争队列时，无锁队列避免了互斥锁的阻塞和上下文切换，延迟更低，吞吐更高。

5.  任务计算轻量的情况下(可能只有几十纳秒到几百纳秒)， 任务提交、队列操作、std::future 的创建、线程调度、缓存一致性等开销远大于任务本身的计算时间。


## 六、总结 & GitHub 链接
在这个C++并发编程阶段的学习和练习感觉学习到了很多，对自身的时间管理和情绪管理也得到了提升。最主要的是学会了一些适合自己的学习方法，这个是对一生都有长期价值的东西。
