...
public:
    ...
    void UpdateRemoteState(uint32_t player_id, Fixed x, Fixed y, uint32_t server_frame);

    // 2. 获取远程玩家当前应渲染的位置（由渲染循环调用）
    //    返回值：true 表示有有效插值结果，out_x/out_y 被填充
    //            false 表示无有效数据，调用方应使用兜底值或跳过渲染
    bool GetRenderPosition(uint32_t player_id, uint32_t render_frame, Fixed& out_x, Fixed& out_y) const;
