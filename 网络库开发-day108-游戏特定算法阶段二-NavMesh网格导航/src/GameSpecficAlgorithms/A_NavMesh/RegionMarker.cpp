...
namespace NavMesh
{
    ...
    {
        if(RegionMode::Grid == mode_)
        {
            return MarkRegionsGrid(map);
        }

        return MarkRegionsConnectivity(map);
    }

    // 输入：GridMap，输出：每个格子的区域 ID（0 表示不可行走）
    std::vector<uint32_t> RegionMarker::MarkRegionsConnectivity(const std::shared_ptr<A_Star::GridMap> map)
    {
        /*
            1. 遍历所有格子，只处理可行走格子
        ...
        */
        ...
        for(uint32_t y = 0; y < height; ++y)
        {
            for(uint32_t x = 0; x < width; ++x)
            {
                ...
                else
                {
                    ...
                    {
                        region_ids[idx] = up_region_id;
                        MergeRegionId(up_region_id, left_region_id, parent);
                    }
                }
            }
        }
        ...
        for(uint32_t y = 0; y < height; ++y)
        {
            for(uint32_t x = 0; x < width; ++x)
            {
                ...
                if(region_id > 0)
                {
                    region_ids[idx] = FindRoot(region_id, parent);
                }
            }
        }
        ...
        std::cout << "[RegionMarker] Connectivity mode: " << region_count_ << " regions" << std::endl;
        return region_ids;
    }


    // 并查集辅助
    uint32_t RegionMarker::FindRoot(uint32_t x, std::vector<uint32_t>& parent)
    {
        uint32_t root = x;
    ...
    void RegionMarker::MergeRegionId(uint32_t b, uint32_t a, std::vector<uint32_t>& parent)
    {
        uint32_t root_a = FindRoot(a, parent);
        uint32_t root_b = FindRoot(b, parent);
        if(root_a != root_b)
        {
     ...
     // 基于网格划分的区域标记（新增）
    std::vector<uint32_t> RegionMarker::MarkRegionsGrid(const std::shared_ptr<A_Star::GridMap> map)
    {
        uint32_t width = map->GetWidth();
        uint32_t height = map->GetHeight();
        std::vector<uint32_t> region_ids(width * height, 0);

        uint32_t grid_w = grid_size_;
        uint32_t grid_h = grid_size_;
        //uint32_t block_width = width / grid_w + (width % grid_w != 0 ?1:0);
        uint32_t block_width = ( width + grid_w -1 ) / grid_w; // 向上取整，由原先的替换成当前这样的更加高效点。
        ...
        for(uint32_t y = 0; y < height; ++y)
        {
            for(uint32_t x = 0; x < width; ++x)
            {
                ...
                // 计算当前格子属于哪个网格块
                uint32_t block_x = x / grid_w;
                uint32_t block_y = y / grid_h;

                uint32_t block_id = block_y * block_width + block_x;
                block_id += 1; // 不能从0开始，所以统一加1，从1开始
                region_ids[idx] = block_id;
            }
        }
        ...
        std::cout << "[RegionMarker] Grid mode: " << region_count_ << " regions (grid_size=" << grid_size_ << ") " << std::endl;
        return region_ids;
    }
