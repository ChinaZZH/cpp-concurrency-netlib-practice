...
7.优缺点：
      ...
      ❌ TaggedPtr 需要保证原子操作，std::atomic<TaggedPtr> 要求 TaggedPtr 是可平凡复制的（通常满足）。
         
         需要双字 CAS（部分平台不支持 x86-64 支持），且 tag 有溢出风险（64 位基本安全）。
...
8.优缺点
    ...
    ❌ 循环引用可能导致内存泄漏（在单向链表中不存在）；shared_ptr 本身有一定开销，注意性能开销。
