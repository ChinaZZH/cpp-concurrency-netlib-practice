...
7.优缺点：
      ...
      ❌ TaggedPtr 需要同时满足以下条件 且 tag 有溢出风险（64 位基本安全）
         保证原子操作;
         
         可以平凡复制的;
         
         运行平台支持双字CAS(部分平台不支持 x86-64 支持)。
...
1. 最佳应用场景： 单向无锁结构（栈/队列）、已使用 shared_ptr 的项目
    ...
    可配合 std::atomic<std::shared_ptr<T>>（C++20 前性能较差，但 C++20 有所改进，并不是真正的无锁，推荐使用版本号的方式）。
...
8.优缺点
    ...
    ❌ 循环引用可能导致内存泄漏（在单向链表中不存在）；shared_ptr 本身有一定开销，注意性能开销。(并不是真正的无锁，推荐使用版本号的方式)
